...
The Quantum Safe Client profile (§5.15 of [KMIP-Prof]) defines the minimum conformance requirements for a KMIP client that interoperates with a [Quantum Safe Server](quantum-safe-server.md). It extends the [Baseline Client](../base-encoding/base-profiles.md) with a single mandatory transport constraint: TLS 1.3. A client that cannot support TLS 1.3 cannot claim this profile.
