...
The Discover Versions TLS 1.2 Authentication Server Profile is the TLS 1.2 variant of the [Basic Discover Versions Server Profile](basic-discover-versions-server-profile.md). The capability is identical — the server supports the Discover Versions operation — but the channel uses the [TLS 1.2 Authentication Suite](../authentication/tls-1-2-authentication-suite.md) rather than the Basic Authentication Suite.
