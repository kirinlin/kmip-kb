...
An SA-SED Client extends the [Baseline Client](../base-encoding/base-profiles.md) with one additional constraint: it should not use Vendor Attributes to duplicate information already expressible in standard KMIP attributes. This keeps the server's attribute schema clean and avoids interoperability breakage when a different client queries the same objects.

## Server

An SA-SED Server extends the [Baseline Server](../base-encoding/base-profiles.md) and adds:
- **Object**: Secret Data
