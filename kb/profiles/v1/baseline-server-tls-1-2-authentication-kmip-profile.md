...
The Baseline Server TLS 1.2 Authentication KMIP Profile is the TLS 1.2 variant of the [Basic Baseline Server KMIP Profile](basic-baseline-server-kmip-profile.md). It carries the full baseline server capability — Create, Register, Get, Get Attributes, Get Attribute List, Locate, Destroy, Query, Discover Versions — while mandating the [TLS 1.2 Authentication Suite](../authentication/tls-1-2-authentication-suite.md) for transport security.
