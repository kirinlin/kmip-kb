...
The Basic Secret Data Server KMIP Profile defines server requirements for managing Secret Data objects in KMIP v1.1 deployments using the [Basic Authentication Suite](../authentication/basic-authentication-suite.md). It extends the [Basic Baseline Server](basic-baseline-server-kmip-profile.md) with explicit Secret Data object support. Secret Data objects hold opaque secret material — passwords, API tokens, PINs, pre-shared keys, or any credential that is not a structured cryptographic key.
