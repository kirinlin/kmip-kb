...
How conformance works in KMIP 1.x: the spec's §12 defers to the separate
OASIS profiles document ([KMIP-Prof]), which defines named server and client
profiles — baseline messaging plus use-case bundles (symmetric key lifecycle,
asymmetric keys, cryptographic services, tape libraries, ...) with test
cases.

- [KMIP Server Implementation Conformance](kmip-server-implementation-conformance.md)
  — what it means for a server to conform (§12.1).
- [KMIP Client Implementation Conformance](kmip-client-implementation-conformance.md)
  — the client-side counterpart (§12.2).
