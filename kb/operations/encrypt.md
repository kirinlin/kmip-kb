...
| Data | `4200C2` | `Data` | Yes (single-part) | The plaintext bytes. Omitted in the continuation calls of a multi-part operation. |
| IV/Counter/Nonce | `42003D` | `IVCounterNonce` | No | The IV, counter, or nonce the mode calls for. |
| Correlation Value | `4200D6` | `CorrelationValue` | No | Ties this call to an in-progress multi-part operation started by an earlier call. |
...
| Authenticated Encryption Additional Data | `4200FE` | `AuthenticatedEncryptionAdditionalData` | No | Extra data covered by the authentication tag (AEAD); for multi-part it goes on the first call. |
...
| IV/Counter/Nonce | `42003D` | `IVCounterNonce` | No | When a random IV was requested and none supplied, the generator output the server used. |
| Correlation Value | `4200D6` | `CorrelationValue` | No | A handle to pass to subsequent calls of a multi-part operation. |
| Authenticated Encryption Tag | `4200FF` | `AuthenticatedEncryptionTag` | No | The AEAD tag, returned once the final block of plaintext has been processed. |
