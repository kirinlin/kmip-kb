...
├── kb/
│   ├── index.go   Scans kb/**/*.md at startup; builds in-memory SQLite FTS5 index
│   └── server.go  MCP tool handlers (thin wrappers over Index methods)
└── raw/
    ├── index.go   Scans raw/**/*.md; same pattern; extracts doc_type/version/is_final from path
...
**Index** is built in memory (`:memory:`) on every startup. Startup time on the full corpus is under 500 ms.
