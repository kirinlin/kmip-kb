...
The Basic Baseline Client KMIP Profile defines the standard client conformance point for KMIP v1.1 deployments using the [Basic Authentication Suite](basic-authentication-suite.md). It is the v1.1 equivalent of the Baseline Client described in [Base Profiles](base-profiles.md) for v2.x, carrying the same core mandatory client-side operations paired with the Basic authentication option.
