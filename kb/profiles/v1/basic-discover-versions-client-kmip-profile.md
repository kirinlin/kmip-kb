...
The Basic Discover Versions Client KMIP Profile defines the minimum client capability for performing version negotiation with a KMIP v1.1 server, using the [Basic Authentication Suite](../authentication/basic-authentication-suite.md). A client claiming this profile can invoke the Discover Versions operation and use the response to determine which KMIP versions and operations the server supports.
